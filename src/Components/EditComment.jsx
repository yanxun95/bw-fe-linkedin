...
export default function EditComment({fetchComments, postId, userId, commentId}) {
  // CONSTANT
...
console.log("THIS IS EDITING COMMENT", EditingComment)
  ...
  // }
   ...
   const editData = async () => {
     try {
      let response = await fetch(`${process.env.REACT_APP_BE_URL}/posts/comment/${commentId}/${userId}`, {
        method: "PUT",
   ...
   const deleteFunction = async (e) => {
     try {
       let response = await fetch(`${process.env.REACT_APP_BE_URL}/posts/comment/${commentId}`, {
         method: "DELETE",
         body: JSON.stringify(EditingComment),
         headers: {
           "Content-type": "application/json; charset=UTF-8"
